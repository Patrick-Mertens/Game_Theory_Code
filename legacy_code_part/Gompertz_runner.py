...
#C, how many times runnning the model
#runs = 3 #for the same seed
seed = 42
#C, the different amount of seeds
N_runs = 2

def run_model(run, studies, functions, splits, noPars, dataset_path, maxi,seed):
    for studyName in studies:
        sind = studies.index(studyName)
        sp = splits[sind]
        ...
        for functionToFit in functions:
                    ...
                    if  'INV-T001' in temp:
                        ...
                        if len(tumorFiltered_Data) >= 6:
                            ...
                            if normalizeDimension:
                                dimension_copy = [d / maxi for d in dimension_copy] #C, changed this to this from the orginal one, becaue it was complaing about it
                            ...
                            if SaveERROR == True:
                                ...
                                target_dir = os.path.join(dataset_path, 'Error_Params_study1to_4', studyName, functionToFit, arm)
            ...
            # Inside your loop
            output_directory = os.path.join(dataset_path, functionToFit, 'Runner_script_study1to4',f"seed_{seed}",f"for_loop_itteration_{run}",'Combo_formula')
...
def main():
    total_runs = 2
    runs = range(1, total_runs + 1)  # This will create a list [1, 2, 3]
    studies = ['1', '2', '3', '4']
    ...
    functions = ['Exponential', 'Logistic', 'ClassicBertalanffy', 'GeneralBertalanffy', 'Gompertz','Gompertz_orginial', 'GeneralGompertz']


    noPars = [3, 3, 3, 4, 3,3, 4]
    maxi = 228

    with concurrent.futures.ProcessPoolExecutor() as executor:
        # Using a list comprehension to start all the runs in parallel
        futures = {executor.submit(run_model, run, studies, functions, splits, noPars, dataset_path, maxi, seed): run for run
                   in runs}
