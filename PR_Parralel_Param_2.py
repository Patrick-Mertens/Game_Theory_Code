...
file_path = os.path.join(dataset_path, "patient_table_3_Fixed_grouping.txt")
...
def worker(study_num, common_indexes, Size, Inc, scaled_pop, scaled_days, list_patients):
    ...
    if study_type:
        target_dir = os.path.join(dataset_path, 'Parallel_code_3_Fixed_groups', study_name, study_type, 'Index_removed')
        if not os.path.exists(target_dir):
            os.makedirs(target_dir)
    else:
        print(f"Invalid study number format for {study_num}. Expected _Inc or _Dec in the name.")
