#C, #################################################### IMPORTANT ###########################################################
#C, This code contains a lot of legacy work. When I started with it I needed to figure it out how it worked and what the
#C, different environement results meant. For this reason, I build a few 'switches' this allows for additional control e.g. saving extra data to control.
#C, For this reason it is recommended heavly to read this scirpt in detail before running the script.
########################################################## END IMPORTANT NOTE ################################
...
paramControl = "4_November" #C, to create a sub folder in the path. To make easier to seprate it when multiple different results are present
...
#C, later the limit is hardcoded, thus checking if it was same or not.
...
#Printing
print(f"lim: {lim}") #C, the same as the other scripts: 0.003316135059100506
print(f"Size1: {Size1}")
print(f"Size2: {Size2}")
print(f"Size3: {Size3}")
print(f"Size4: {Size4}")
...
#C, old Legacy work, not needed but kept to be transparent
# noPars=[3,3,3,3,3,3]
...
#C, Trend_resistance, basicaly allows to print every plot in regards to resistance type (in regards to statement), but it could lead to plots
#C, That you dont need. So, it is adivsed to use this carefully
...
for studyName in studies:
    ...
    warnings.filterwarnings("ignore")
    normalizeDimension = True
    #lim = limits[sind] #C, not neeeded
    lim= 0.003316135059100506 #C, This is the same as the limit from study 1 to 4
                ...
                if  'INV-T001' in temp :
                    ...
                    if len(tumorFiltered_Data) >= 6:
                        ...
                        try:  # C, this try box need to move to the left, so that it is in the body of the
                            ...
                            ######################################IMPORTANT################################
                            #C, the fucntion PR_separate_by_size_chemo_vs_immuno is the non-legacy version of the function
                            #C, The legacy version of this function is called PR_separate_by_size, the reason of it being PR it has even an older itteration of the function.
                            #C, that was not operatale in the current computational enviorement
                            ...
                            if Test == 1:
                                ...
                                output_file_path = os.path.join(target_dir, 'Check_split1_ind_and_lim.txt')

                                #C, this line was orginalyl with PR_split1_ind, instead of split1_ind. difference is in handeling of increase and decrease. This was more curiosity
                                PR_Size1, PR_Size2, PR_Size3, PR_Size4, PR_Up, PR_Down, PR_Fluctuate, PR_Evolution, PR_Inc, PR_Dec = split1_ind(lim /20, lim / 2, lim *3, dimension, trend)
