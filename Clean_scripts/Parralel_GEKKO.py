#This script is for the bootstrappign and to get parameters that are rapported in Table 2, furthermore the resulting parameters
#Of the total group (see code) get coppied and hard coded for other scripts.
import numpy as np
...
paramControl = "Control_20" #C, extra control on desetination of Saved files
#Initial settings

#C, Functions is not neeeded, but the for loop is kept, and to prevent to rewrite the entire loop, only one function is selected, does not matter which one
...
splits = [True, True, False, True, True]
noPars = [3, 3, 3, 4, 3, 4]  #C, NoPars is only relevant for full fucntions array,E.g. it is needed for AIC caculation
list_trends=[]
...
#This is for getting the paramaters
studies_data = [
...
if __name__ == '__main__':
    ...
    if run_Total == True:
        ...
        with Pool(processes=cpu_count()-1) as pool: #C, I removed 1 CPU to have one free for the cleanup script
